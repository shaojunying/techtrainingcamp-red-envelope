...
func main() {
	...
	c, err := rocketmq.NewPushConsumer(
		...
		consumer.WithGroupName("GID_001"),
	...
	err = c.Subscribe("snatch_history", consumer.MessageSelector{}, func(ctx context.Context,
		...
		for _, msg := range msgs {
			...
			uid, _ := strconv.Atoi(message_body[0])
			pid, _ := strconv.Atoi(message_body[1])
			message_time := msg.BornTimestamp / 1000
			log.Printf("Subscribe Callback: %v \n", msg)
			if uid < 0 || pid < 0 {
				log.Println("Illegal Data. Dry Run. Not submitted to SQL.")
			} else {
				tmp := dbconnect.RedEnvelope{EnvelopeID: &pid, UID: &uid, SnatchTime: int(message_time)}
				err := tmp.SnatchSql()
				if err != nil {
